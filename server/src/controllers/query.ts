...
export async function getAvg(req: Request, res: Response): Promise<void> {
    ...
    try {        
        if (!deptList || deptList.length === 0) {
            res.status(StatusCodes.BAD_REQUEST).json({ error: 'Provide at least one department.' });
            return;
        }
        
        if (!levelList || levelList.length === 0) {
            res.status(StatusCodes.BAD_REQUEST).json({ error: 'Provide at least one course level.' });
            return;
        }
        ...
        const levelConditions = levelList.map((level: number) => {
        ...
        const query = `
            ...
            ORDER BY dept, id;
        `
        const result = await pool.query(query, [deptList, yearStart, yearEnd, process.env.PGOVERALL_YEAR]);
